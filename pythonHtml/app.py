...
# preparing dataset
url = 'https://drive.google.com/file/d/1bUGxO-HIP8kV3MK_22g7mxdLnmVC1nZ4/view?usp=sharing'
url = 'https://drive.google.com/uc?id=' + url.split('/')[-2]
table_data = pd.read_csv(url)

# processing dataframe
data_content_based_filtering = table_data.copy()
data_content_based_filtering['Tags'] = data_content_based_filtering['Category_en'] + ' ' + data_content_based_filtering[
    'Description_en']
data_content_based_filtering = data_content_based_filtering[
    ['Place_Id', 'Place_Name', 'Tags', 'Category_en', 'City', 'Description_en', 'Lat', 'Long', 'Rating', 'Price']]
...
def recommend_by_content_based_filtering(categories, regions, number_of_locations):
    data_sample = data_content_based_filtering[data_content_based_filtering['Category_en'] == categories].sample(1).values[0][1]
...
def makeRecommender(recommendation):
    ...
    for places in recommendation:
        ...
        search_format += "<h3><a href='https://www.google.com/search?q=" + str(place_name) + "' target='_blank'>" + str(
            place_name) + " </a></h3>" + "<p>Category: " + place_category + ". Entry price (Ghc): " + place_entry_price + ". Rating: " + place_rating + "</p>" + "Description:<br><p>" + str(
            place_desc) + "</p><br><br>"
